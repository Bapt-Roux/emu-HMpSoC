/**
 * @file backprop.cc
 * @brief Implementation of emu-hmpsoc BACKPROP functions.
...
void soft_max(TYPE_BP net_outputs[possible_outputs], TYPE_BP activations[possible_outputs]) {
    int i;
    TYPE_BP sum;
    sum = (TYPE_BP) 0.0;
...
void RELU(TYPE_BP activations[nodes_per_layer], TYPE_BP dactivations[nodes_per_layer], int size) {
    int i;
    for( i = 0; i < size; i++) {
...
void add_bias_to_activations(TYPE_BP biases[nodes_per_layer],
                               TYPE_BP activations[nodes_per_layer],
                               int size) {
    int i;
    for( i = 0; i < size; i++){
...
void matrix_vector_product_with_bias_input_layer(TYPE_BP biases[nodes_per_layer],
                                                 TYPE_BP weights[input_dimension*nodes_per_layer],
                                                 TYPE_BP activations[nodes_per_layer],
                                                 TYPE_BP input_sample[input_dimension]){
    int i,j;
    for(j = 0; j < nodes_per_layer; j++){
        activations[j] = (TYPE_BP)0.0;
...
void matrix_vector_product_with_bias_second_layer(TYPE_BP biases[nodes_per_layer],
                                                 TYPE_BP weights[nodes_per_layer*nodes_per_layer],
                                                 TYPE_BP activations[nodes_per_layer],
                                                 TYPE_BP input_activations[nodes_per_layer]){
    int i,j;
    for (i = 0; i < nodes_per_layer; i++){
        activations[i] = (TYPE_BP)0.0;
        for(j = 0; j < nodes_per_layer; j++){
...
void matrix_vector_product_with_bias_output_layer(TYPE_BP biases[possible_outputs],
                                                 TYPE_BP weights[nodes_per_layer*possible_outputs],
                                                 TYPE_BP activations[possible_outputs],
                                                 TYPE_BP input_activations[nodes_per_layer]){
    int i, j;
...
void take_difference(TYPE_BP net_outputs[possible_outputs],
                     TYPE_BP solutions[possible_outputs],
                     TYPE_BP output_difference[possible_outputs],
                     TYPE_BP dactivations[possible_outputs]) {
    int i;
    for( i = 0; i < possible_outputs; i++){
...
void get_delta_matrix_weights3(TYPE_BP delta_weights3[nodes_per_layer*possible_outputs],
                               TYPE_BP output_difference[possible_outputs],
                               TYPE_BP last_activations[nodes_per_layer]) {
    int i, j;
    for( i = 0; i < nodes_per_layer; i++) {
        for( j = 0; j < possible_outputs; j++) {
...
void get_oracle_activations2(TYPE_BP weights3[nodes_per_layer*possible_outputs],
                             TYPE_BP output_differences[possible_outputs],
                             TYPE_BP oracle_activations[nodes_per_layer],
                             TYPE_BP dactivations[nodes_per_layer]) {
    int i, j;
    for( i = 0; i < nodes_per_layer; i++) {
        oracle_activations[i] = (TYPE_BP)0.0;
        for( j = 0; j < possible_outputs; j++) {
...
void get_delta_matrix_weights2(TYPE_BP delta_weights2[nodes_per_layer*nodes_per_layer],
                               TYPE_BP output_difference[nodes_per_layer],
                               TYPE_BP last_activations[nodes_per_layer]) {
    int i, j;
    for( i = 0; i < nodes_per_layer; i++) {
        for( j = 0; j < nodes_per_layer; j++) {
...
void get_oracle_activations1(TYPE_BP weights2[nodes_per_layer*nodes_per_layer],
                             TYPE_BP output_differences[nodes_per_layer],
                             TYPE_BP oracle_activations[nodes_per_layer],
                             TYPE_BP dactivations[nodes_per_layer]) {
    int i, j;
    for( i = 0; i < nodes_per_layer; i++) {
...
void get_delta_matrix_weights1(TYPE_BP delta_weights1[input_dimension*nodes_per_layer],
                               TYPE_BP output_difference[nodes_per_layer],
                               TYPE_BP last_activations[input_dimension]) {
    int i, j;
    for( i = 0; i < input_dimension; i++) {
        for( j = 0; j < nodes_per_layer; j++) {
...
void update_weights(TYPE_BP weights1[input_dimension*nodes_per_layer],
                    TYPE_BP weights2[nodes_per_layer*nodes_per_layer],
                    TYPE_BP weights3[nodes_per_layer*possible_outputs],
                    TYPE_BP d_weights1[input_dimension*nodes_per_layer],
                    TYPE_BP d_weights2[nodes_per_layer*nodes_per_layer],
                    TYPE_BP d_weights3[nodes_per_layer*possible_outputs],
                    TYPE_BP biases1[nodes_per_layer],
                    TYPE_BP biases2[nodes_per_layer],
                    TYPE_BP biases3[possible_outputs],
                    TYPE_BP d_biases1[nodes_per_layer],
                    TYPE_BP d_biases2[nodes_per_layer],
                    TYPE_BP d_biases3[possible_outputs]) {
    int i, j;
...
void backprop(TYPE_BP weights1[input_dimension*nodes_per_layer],
                TYPE_BP weights2[nodes_per_layer*nodes_per_layer],
                TYPE_BP weights3[nodes_per_layer*possible_outputs],
                TYPE_BP biases1[nodes_per_layer],
                TYPE_BP biases2[nodes_per_layer],
    ...
    TYPE_BP activations1[nodes_per_layer];
    TYPE_BP activations2[nodes_per_layer];
    TYPE_BP activations3[possible_outputs];
    TYPE_BP dactivations1[nodes_per_layer];
    TYPE_BP dactivations2[nodes_per_layer];
    TYPE_BP dactivations3[possible_outputs];
    TYPE_BP net_outputs[possible_outputs];
    //Training structure
    TYPE_BP output_difference[possible_outputs];
    TYPE_BP delta_weights1[input_dimension*nodes_per_layer];
    TYPE_BP delta_weights2[nodes_per_layer*nodes_per_layer];
    TYPE_BP delta_weights3[nodes_per_layer*possible_outputs];
    TYPE_BP oracle_activations1[nodes_per_layer];
    TYPE_BP oracle_activations2[nodes_per_layer];

    for(i=0; i<training_sets; i++){
        for(j=0;j<nodes_per_layer;j++){
            activations1[j] = (TYPE_BP)0.0;
            activations2[j] = (TYPE_BP)0.0;
            if(j<possible_outputs){
                activations3[j] = (TYPE_BP)0.0;
            }
        }
