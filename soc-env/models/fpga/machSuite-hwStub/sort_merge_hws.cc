/**
 * @file sort_merge_hws.cc
 * @brief HW Stub for SORT_MERGE from machsuite.
 */
#include "sort_merge_hws.h"

/**
 * @brief emu-hmpsoc machsuite sort_merge HW_Stub constructor.
 ...
 */
sort_merge_hwStub::sort_merge_hwStub(sc_core::sc_module_name name, uint compTime_ns, uint compEn_nj)
...
sort_merge_hwStub::~sort_merge_hwStub(){
}

/**
 * @brief emu-hmpsoc SC_Thread implementation for machsuite sort_merge HW_Stub .
 */
void sort_merge_hwStub::parse_cmd(){
  wait(); // prevent execution at bootup
  while(1) {
#ifdef MACHSUITE_DEBUG
    std::cout << "@ "<< sc_time_stamp() << " ==>"<< "SORT_MERGE received cmd." << std::endl;
#endif
        ...
        *((uint8_t*)_REG(STATUS)) &= ~(STATUS_DONE + STATUS_IRQ(0));
#ifdef MACHSUITE_DEBUG
        std::cout << "SORT_MERGE_HW started for "<< (uint32_t) *((uint32_t*)_REG(PARAMS)) <<" iterations\n";
        ...
        for( uint i=0; i< *((uint32_t*)_REG(PARAMS)); i++){
          ...
          memRead(*((uint32_t*)_REG(ARRAY_IN))
                  + i*(sizeof(TYPE_SM)* SIZE), data, sizeof(TYPE_SM)*(SIZE), 0);
          ms_mergesort(data);
          wait(cTime_ns, SC_NS);
